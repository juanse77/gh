...
def test_get_numerical_features_empty():
    ...
    df = pd.DataFrame({"categorica1": ["azul"], "categorica2": ["rojo"]})
...
def test_get_numerical_features_zero_rows():
    ...
    assert get_numerical_features(pd.DataFrame({"numerica": pd.Series(dtype=int)})) == [
        "numerica"
    ]
...
def test_get_numerical_features_floats():
    ...
    df = pd.DataFrame({"numerica": [5], "numerica2": [5.2]})
...
def test_get_numerical_features_complex():
    ...
    """
    df = pd.DataFrame({"complejo": [complex(3, 5)]})
